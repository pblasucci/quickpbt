...
module Filtered =
  let zoneName =
    Platform.As
      (win  = fun () -> "Eastern Standard Time"
      ,osx  = fun () -> "America/New_York"
  ...
  let ``daylight savings test oracle (naive)`` (anyDate :Date) =
    ...
    *)
    let eastern   = Zone.FindSystemTimeZoneById(zoneName)
  ...
  let ``zone is unchanged through round-trip serialization`` () =
    ...
    let test (z :Zone) =
      if Platform.IsWin
        then  let deflated = z.ToSerializedString()
    ...
    Prop.forAll zones (fun z -> lazy (test z))
